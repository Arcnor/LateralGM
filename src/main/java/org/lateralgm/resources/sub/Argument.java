	...
	public String toString(LibArgument la) {
		...
		switch (kind) {
			...
			case Argument.ARG_MENU:
				...
				} catch (NumberFormatException nfe) { //Silly user. Return default val
				} catch (IndexOutOfBoundsException be) { //Silly user. Return default val
				}
				return val;
			case Argument.ARG_COLOR:
				try {
					return String.format("%06X", Integer.parseInt(val));
				} catch (NumberFormatException e) { //Silly user. Return default val
				}
				return val;
			default:
