/*
 * VTextIcon by Lee Ann Rucker, modified by Quadduc
 *  Anybody can use the code for any purpose; I don't want any compensation,
	...
	void calcDimensions() {
		...
		if (fRotation == ROTATE_NONE) {
			...
			for (int i = 0; i < len; i++) {
				...
				if (IN_TOP_RIGHT.indexOf(ch) >= 0) // if ch is in sDrawsInTopRight
					fPosition[i] = POSITION_TOP_RIGHT;
				else if (IN_FAR_TOP_RIGHT.indexOf(ch) >= 0)
