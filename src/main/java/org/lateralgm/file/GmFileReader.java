...
public final class GmFileReader {
	...
	private static void readSounds(ProjectFileContext c) throws IOException, GmFormatException,
		...
		for (int i = 0; i < noSounds; i++) {
			...
			if (ver == 800) in.skip(8); //last changed
			ver = in.read4();
			if (ver != 440 && ver != 600 && ver != 800)
				throw versionError(f, "IN", "SND", i, ver); //$NON-NLS-1$ //$NON-NLS-2$
			int kind53 = -1;
			if (ver == 440)
	...
	private static void readSprites(ProjectFileContext c) throws IOException, GmFormatException,
		...
		if (ver != 400 && ver != 800 && ver != 810)
			throw versionError(f, "BEFORE", "SPR", ver); //$NON-NLS-1$ //$NON-NLS-2$
			...
			for (int j = 0; j < nosub; j++) {
				if (ver >= 800) {
					...
					if (subver != 800 && subver != 810)
						throw versionError(f, "IN", "SPR", i, subver); //$NON-NLS-1$ //$NON-NLS-2$
					w = in.read4();
					h = in.read4();
	...
	private static int readBackgrounds(ProjectFileContext c) throws IOException, GmFormatException,
		...
		for (int i = 0; i < noBackgrounds; i++) {
			...
			if (ver == 800) in.skip(8); //last changed
			ver = in.read4();
			if (ver != 400 && ver != 543 && ver != 710)
				throw versionError(f, "IN", "BKG", i, ver); //$NON-NLS-1$ //$NON-NLS-2$
			if (ver < 710) {
				int w = in.read4();
				int h = in.read4();
	...
	private static void readScripts(ProjectFileContext c) throws IOException, GmFormatException {
		...
		int ver = in.read4();
		if (ver != 400 && ver != 800 && ver != 810)
			throw versionError(f, "BEFORE", "SCR", ver); //$NON-NLS-1$ //$NON-NLS-2$
		...
		for (int i = 0; i < noScripts; i++) {
			...
			ver = in.read4();
			if (ver != 400 && ver != 800 && ver != 810)
				throw versionError(f, "IN", "SCR", i, ver); //$NON-NLS-1$ //$NON-NLS-2$
