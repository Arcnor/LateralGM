...
import javax.swing.text.rtf.RTFAttributesExt.AssertiveAttribute;
...
class RTFGeneratorExt extends Object {
	/**
    ...
    /* Write the document properties */
		...
		for (i = 0; i < RTFAttributesExt.attributes.length; i++) {
			RTFAttributeExt attr = RTFAttributesExt.attributes[i];
			if (attr.domain() != RTFAttributeExt.D_DOCUMENT)
				continue;
	...
	void updateSectionAttributes(MutableAttributeSet current,
		...
		checkControlWords(current, newAttributes,
				RTFAttributesExt.attributes, RTFAttributeExt.D_SECTION);
	}
...
*/
    ...
    /* The only way to get rid of tabs or styles is with the \pard keyword,
       ...
       paragraph's tabs. */
		...
		checkControlWords(current, newAttributes,
				RTFAttributesExt.attributes, RTFAttributeExt.D_PARAGRAPH);
	...
	void updateCharacterAttributes(MutableAttributeSet current,
		...
		checkControlWords(current, newAttributes,
				RTFAttributesExt.attributes, RTFAttributeExt.D_CHARACTER);
		...
		if ((parm = attrDiff(current, newAttributes,
				StyleConstants.Background, null, MagicToken)) != null) {
			int colorNum;
			if (parm == MagicToken)
				colorNum = 0;
			else
		...
		if ((parm = attrDiff(current, newAttributes,
				StyleConstants.Foreground, null, null)) != null) {
			int colorNum;
			if (parm == MagicToken)
				colorNum = 0;
			else
