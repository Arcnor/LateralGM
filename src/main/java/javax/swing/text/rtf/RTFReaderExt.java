 ...
 */
class RTFReaderExt extends RTFParserExt {
	...
	static {
		straightforwardAttributes = RTFAttributesExt.attributesByKeyword();
	}
...
/* TODO: per-font font encodings ( \fcharset control word ) ? */
	...
	public RTFReaderExt(StyledDocument destination) {
		...
		target = destination;
		parserState = new Hashtable<String, Object>();
		fontTable = new Hashtable<Object, String>();

		rtfversion = -1;

		mockery = new MockAttributeSetExt();
		...
		if (set == null) {
			...
			charsetStream = (InputStream) java.security.AccessController.
					doPrivileged(new java.security.PrivilegedAction() {
						public Object run() {
							return RTFReaderExt.class.getResourceAsStream
									("charsets/" + name + ".txt");
						}
					});
...
//          continue;
    ...
    /* we do this little dance to avoid cloning the entire state stack and
       immediately throwing it away. */
		...
		Dictionary<String, Object> saveState = (Dictionary<String, Object>) ((Hashtable<String, Object>) parserState).clone();
		if (oldSaveState != null)
	...
	public void endgroup() {
		...
		Dictionary<String, Object> restoredState = (Dictionary<String, Object>) parserState.get("_savedState");
		...
		Dictionary<String, Object> oldParserState = parserState;
	 ...
	 * @see RTFParserExt#handleKeyword
	 */
...
//    target.changeAttributes(new LFDictionary(LFArray.arrayWithObject(value), LFArray.arrayWithObject(name)));
	...
	interface Destination {
		...
		void endgroup(Dictionary<String, Object> oldState);
	...
	class FonttblDestination implements Destination {
		...
		public void close() {
			Enumeration<Object> nums = fontTable.keys();
	...
	abstract class AttributeTrackingDestination implements Destination {
		...
		public boolean handleKeyword(String keyword) {
...

			{
				...
				if (item != null) {
					...
					switch (attr.domain()) {
						case RTFAttributeExt.D_CHARACTER:
							ok = attr.set(characterAttributes);
							break;
						case RTFAttributeExt.D_PARAGRAPH:
							ok = attr.set(paragraphAttributes);
							break;
						case RTFAttributeExt.D_SECTION:
							ok = attr.set(sectionAttributes);
							break;
						case RTFAttributeExt.D_META:
							mockery.backing = parserState;
							ok = attr.set(mockery);
							mockery.backing = null;
							break;
						case RTFAttributeExt.D_DOCUMENT:
        ...
        /* TODO: Other kinds of underlining */
			...
			if (keyword.equals("tx") || keyword.equals("tb")) {
				...
				Dictionary<Comparable, Serializable> tabs;
				Integer stopCount;

				tabs = (Dictionary<Comparable, Serializable>) parserState.get("_tabs");
				if (tabs == null) {
		...
		protected void resetCharacterAttributes() {
			...
			while (attributes.hasMoreElements()) {
				...
				if (attr.domain() == RTFAttributeExt.D_CHARACTER)
		...
		protected void resetParagraphAttributes() {
			...
			while (attributes.hasMoreElements()) {
				RTFAttributeExt attr = (RTFAttributeExt) attributes.nextElement();
				if (attr.domain() == RTFAttributeExt.D_PARAGRAPH)
					attr.setDefault(characterAttributes);
			}
		}
		...
		protected void resetSectionAttributes() {
			...
			while (attributes.hasMoreElements()) {
				RTFAttributeExt attr = (RTFAttributeExt) attributes.nextElement();
				if (attr.domain() == RTFAttributeExt.D_SECTION)
					attr.setDefault(characterAttributes);
			}
